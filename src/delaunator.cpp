...
delaunator::delaunator() :
    triangles(), halfedges(), hull_start(0), coords(), hull_prev(),
...
void delaunator::_triangulate(const std::span<const terra::vec2>& in_coords)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        const tfloat d = glm::distance2(c, this->coords[i]);
        if (d < min_dist)
        {
            i0 = i;
            min_dist = d;
    ...
    for (size_t i = 0; i < n; ++i)
    {
        if (i == i0)
            continue;
        const tfloat d = glm::distance2(i0v, this->coords[i]);
        if (d < min_dist && d > 0.0)
        {
            i1 = i;
            min_dist = d;
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        const tfloat r = circumradius(i0v, i1v, coords[i]);
        if (r < min_radius)
        {
            i2 = i;
            min_radius = r;
    ...
    for (size_t k = 0; k < n; k++)
    {
        ...
        const auto current_point = this->coords[i];
        ...
        while (q = hull_next[e],
               !orient(current_point, this->coords[e], coords[q]))
        {
            e = q;
            if (e == start)
            {
        ...
        while (q = hull_next[next],
               orient(current_point, this->coords[next], this->coords[q]))
        {
            t = add_triangle(
...
tfloat delaunator::get_hull_area()
{
    ...
    do
    {
        const auto ev = this->coords[e];
        const auto pv = this->coords[hull_prev[e]];
...
size_t delaunator::legalize(size_t a)
{
    ...
    while (true)
    {
        ...
        const bool illegal = in_circle(this->coords[p0],
                                       this->coords[pr],
                                       this->coords[pl],
                                       this->coords[p1]);
