...
hash_grid::hash_grid(int64_t size_x, int64_t size_y, double radius)
{
    ...
    this->grid_size_x = utils::fast_ceil<int64_t>(static_cast<double>(size_x) / bucket_size);
    this->grid_size_y = utils::fast_ceil<int64_t>(static_cast<double>(size_y) / bucket_size);
...
constexpr size_t HashPos(const terra::vec2& point, const double bucket_size, const int64_t grid_size_x)
{
    int64_t x = std::max<int64_t>(static_cast<int64_t>(utils::fast_floor<double>(point.x) / bucket_size), 0);
    int64_t y = std::max<int64_t>(static_cast<int64_t>(utils::fast_floor<double>(point.y) / bucket_size), 0);
...
std::vector<int64_t> hash_grid::neighbours(const terra::vec2& point)
{
    ...
    int64_t x = static_cast<int64_t>(utils::fast_floor<double>(point.x) / this->bucket_size);
    int64_t y = static_cast<int64_t>(utils::fast_floor<double>(point.y) / this->bucket_size);
