...
#ifndef TERRA_DYNARRAY_HPP
#define TERRA_DYNARRAY_HPP
...
template<typename T>
auto terra::dynarray<T>::operator=(dynarray const& other) -> dynarray & {
    if (size() != other.size()) {
...
template<typename T>
auto terra::dynarray<T>::operator=(std::initializer_list<T> list) -> dynarray & {
    if (size() != list.size()) {
        using namespace std::string_literals;
        throw std::invalid_argument{
...
template<typename T>
auto terra::dynarray<T>::at(size_type pos) -> reference {
    if (pos >= size()) {
        using namespace std::string_literals;
...
template<typename T>
auto terra::dynarray<T>::at(size_type pos) const -> const_reference {
    if (pos >= size()) {
        using namespace std::string_literals;
        throw std::out_of_range{
            "cannot access element at position "s + 
...
template<typename T>
auto terra::dynarray<T>::operator[](size_type pos) -> reference {
    return m_data[pos];
}

template<typename T>
auto terra::dynarray<T>::operator[](size_type pos) const -> const_reference {
    return m_data[pos];
}

template<typename T>
auto terra::dynarray<T>::front() -> reference {
    return m_data[0];
}

template<typename T>
auto terra::dynarray<T>::front() const -> const_reference {
    return m_data[0];
}

template<typename T>
auto terra::dynarray<T>::back() -> reference {
    return m_data[size() - 1];
}

template<typename T>
auto terra::dynarray<T>::back() const -> const_reference {
    return m_data[size() - 1];
}

template<typename T>
auto terra::dynarray<T>::data() -> pointer {
    return m_data.get();
}

template<typename T>
auto terra::dynarray<T>::data() const -> const_pointer {
    return m_data.get();
}
...
template<typename T>
auto terra::dynarray<T>::empty() const -> bool {
    return m_size == 0;
}

template<typename T>
auto terra::dynarray<T>::size() const -> size_type {
    return m_size;
}
...
template<typename T>
auto terra::dynarray<T>::begin() -> iterator {
    return m_data.get();
}

template<typename T>
auto terra::dynarray<T>::begin() const -> const_iterator {
    return m_data.get();
}

template<typename T>
auto terra::dynarray<T>::cbegin() const -> const_iterator {
    return m_data.get();
}


template<typename T>
auto terra::dynarray<T>::end() -> iterator {
    return m_data.get() + size();
}

template<typename T>
auto terra::dynarray<T>::end() const -> const_iterator {
    return m_data.get() + size();
}

template<typename T>
auto terra::dynarray<T>::cend() const -> const_iterator {
    return m_data.get() + size();
}
