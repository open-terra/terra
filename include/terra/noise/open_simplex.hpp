...
namespace terra::noise
{
    ...
    {
    private:
        const tfloat STRETCH_CONSTANT_2D =
            -0.211324865405187; // (1 / sqrt(2 + 1) - 1 ) / 2;
        const tfloat SQUISH_CONSTANT_2D =
            0.366025403784439; // (sqrt(2 + 1) -1) / 2;
        const tfloat STRETCH_CONSTANT_3D =
            -1.0 / 6.0; // (1 / sqrt(3 + 1) - 1) / 3;
        const tfloat SQUISH_CONSTANT_3D = 1.0 / 3.0; // (sqrt(3+1)-1)/3;
        const tfloat STRETCH_CONSTANT_4D =
            -0.138196601125011; // (1 / sqrt(4 + 1) - 1) / 4;
        const tfloat SQUISH_CONSTANT_4D =
            0.309016994374947; // (sqrt(4 + 1) - 1) / 4;

        const tfloat NORM_CONSTANT_2D = 47.0;
        const tfloat NORM_CONSTANT_3D = 103.0;
        const tfloat NORM_CONSTANT_4D = 30.0;
    ...
    private:
        constexpr tfloat extrapolate2(int xsb, int ysb, tfloat dx, tfloat dy);
        constexpr tfloat extrapolate3(int xsb,
                                      int ysb,
                                      int zsb,
                                      tfloat dx,
                                      tfloat dy,
                                      tfloat dz);
        constexpr tfloat extrapolate4(int xsb,
                                      int ysb,
                                      int zsb,
                                      int wsb,
                                      tfloat dx,
